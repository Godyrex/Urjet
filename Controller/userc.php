...
?>
  ...
  function verification($verify)
  {
    ...
    if ($row > 1) {
      ...
      $stmt->execute();
      $link = "http://localhost/project/Integre/verify-email.php?key=" . $verify . "&token=" . $token . "";
      $receiver = $verify;
 ...
 setcookie ("rememberme","", time() - ($days * 24 * 60 * 60 * 1000));
  ...
  function reset($emailId, $userc)
  {
    ...
    if ($row) {
      ...
      $update->execute();
      $link = "http://localhost/project/Integre/reset-password.php?key=" . $emailId . "&token=" . $token . "";
