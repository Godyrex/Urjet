        ...
        <div class="col">
          <div class="card">
            ...
            <div class="card-header border-0">
              <h3 class="mb-0">La liste des aeroports :</h3>
            </div>
            <!-- Light table -->
...
<?php //on inclut notre fichier de connection $pdo = Database::connect(); //on se connecte à la base $sql = 'SELECT * FROM user ORDER BY id DESC'; //on formule notre requete foreach ($pdo->query($sql) as $row) { //on cree les lignes du tableau avec chaque valeur retournée
    include "../Controller/aeroportC.php";
      $aeroport = new aeroportC();
    $resultat=$aeroport->afficher_aeroport();
    ...
    <td>' . $row['ville'] . '</td>
      ...
      <td>' . $row['pays'] . '</td>
       <p>';
          ...
          echo '<td>';
         echo '<a class="btn btn-success" href="modifier_aeroport.php?id_aeroport=' . $row['id_aeroport'] . '">Update</a>';// un autre td pour le bouton d'update
       
                      echo '<a class="btn btn-primary" href="Supprimer_aeroport.php?id_aeroport=' . $row['id_aeroport'] . ' ">Delete</a>';// un autre td pour le bouton de suppression
